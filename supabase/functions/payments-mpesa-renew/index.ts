...
serve(async (_req) => {
  ...
  const { data: subs, error } = await supabase.from('user_subscriptions')
    .select('user_id, tier, phone_number')
    .eq('payment_provider', 'mpesa').eq('auto_renew', true)
    .lte('expires_at', renewBefore.toISOString())
    .gt('expires_at', new Date().toISOString());

  if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500 });
  if (!subs || subs.length === 0) return new Response(JSON.stringify({ renewed: 0 }), { status: 200 });

  const tokenResp = await fetch(
    'https://api.safaricom.co.ke/oauth/v1/generate?grant_type=client_credentials',
    { headers: { Authorization: 'Basic ' + btoa(`${DARAJA_CONSUMER_KEY}:${DARAJA_CONSUMER_SECRET}`) } }
  );
  ...
  for (const sub of subs) {
    ...
    try {
      const stkResp = await fetch('https://api.safaricom.co.ke/mpesa/stkpush/v1/processrequest', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          PartyA: sub.phone_number, PartyB: DARAJA_SHORTCODE, PhoneNumber: sub.phone_number,
      ...
      if (stkData.ResponseCode === '0') {
        await supabase.from('payment_transactions').insert({
          user_id: sub.user_id, amount, currency: 'KES', provider: 'mpesa',
          status: 'pending', tier: sub.tier, provider_ref: stkData.CheckoutRequestID,
