...
serve(async (req) => {
  ...
  try {
    ...
    await supabase.from('payment_transactions').insert({
      user_id: user.id, amount: Number(amount), currency: currency ?? 'KES',
      provider: 'mpesa', status: 'pending', tier,
      provider_ref: stkData.CheckoutRequestID, metadata: { phone, timestamp },
    });
