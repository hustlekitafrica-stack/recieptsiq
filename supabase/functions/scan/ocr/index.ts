...
/** Safe base64 encoding for large binary buffers (avoids call-stack overflow). */
...
serve(async (req) => {
  ...
  try {
    ...
    // ── Download temp image from Storage (service role bypasses RLS) ────────
    ...
    // ── Delete temp file (best-effort; failure is non-fatal) ────────────────
    await supabase.storage.from('ocr-temp').remove([storage_path]).catch(() => {});

    // ── Parse response ──────────────────────────────────────────────────────
    const response0 = visionData.responses?.[0];
    if (response0?.error) {
      throw new Error(`Vision API error: ${response0.error.message}`);
    }

    const text: string = response0?.fullTextAnnotation?.text ?? '';
    if (!text.trim()) {
      throw new Error('No text detected in this image.');
    }
